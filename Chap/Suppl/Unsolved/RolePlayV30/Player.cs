...
public class Player
{
	...
	{
		// Tjek om det nye stykke Armor allerede er i _armorList.
		if (_armorList.Select(a => a.Id).Contains(armor.Id))
		{
	...
	{
		return _armorList
			.Select(a => a.GetResistance(damageType))
			.Sum();
	}
	...
	/// </summary>
	private double CalculateResultingDamage(Damage damage) //MULIGT
	{
		double nettoDamage = 0;
		foreach (var item in damage.AsDictionary())
		{
			double incomingDamage = item.Value;
			DamageType dmgType = item.Key;

			double resistance = GetResistance(dmgType);
			double factor = ResistanceToDamageFactor(resistance);
			double nettoDamageFromDamageType = incomingDamage * factor;

			nettoDamage = nettoDamage + nettoDamageFromDamageType;
		}


		return damage
			.AsDictionary()
			.Select(item => item.Value * ResistanceToDamageFactor(GetResistance(item.Key)))
			.Sum();
	}
}
