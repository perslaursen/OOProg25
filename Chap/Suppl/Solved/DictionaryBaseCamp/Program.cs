...
Console.WriteLine("Case 5, first version");
...
Console.WriteLine("Case 5, second version");
foreach (var testScore in testScores)
{
	Console.WriteLine($"{testScore.Key} has test score {testScore.Value}");
}
Console.WriteLine();
...
foreach (var testScore in testScores)
{
	sum = sum + testScore.Value;
}
int average = sum / testScores.Count;
Console.WriteLine($"Case 6, average of all scores is {average}");
Console.WriteLine();
...
int bestSoFar = 0;
string nameOfBestSoFar = "(undefined)";
foreach (var testScore in testScores)
{
	if (testScore.Value > bestSoFar)
	{
		bestSoFar = testScore.Value;
		nameOfBestSoFar = testScore.Key;
	}
}
Console.WriteLine($"Case 7, {nameOfBestSoFar} has the highest test score ({bestSoFar})");
Console.WriteLine();
